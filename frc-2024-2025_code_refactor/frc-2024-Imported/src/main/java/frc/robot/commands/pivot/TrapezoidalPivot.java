...
public class TrapezoidalPivot extends Command {
  ...
  public TrapezoidalPivot(Pivot pivot, double goal) {
    SmartDashboard.putNumber("pivot goal", goal);
    m_pivot = pivot;
  ...
  public void execute() {
    ...
    ff = 0;
    SmartDashboard.putNumber("Pivot setpoint", m_sample.position);
    SmartDashboard.putNumber("Pivot position", m_pivot.getEncoderPosition());
    //double pid = m_pid.calculate(m_pivot.getEncoderPosition(), m_sample.position);

    double pid = m_pid.calculate(m_pivot.getEncoderPosition(), m_goal.position);
    double speed = ff + pid;
