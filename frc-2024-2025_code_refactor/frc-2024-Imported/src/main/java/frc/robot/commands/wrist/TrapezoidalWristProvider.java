 ...
 */
public class TrapezoidalWristProvider extends Command {
  ...
  private Supplier<Double> m_goalSupplier;
  ...
  public TrapezoidalWristProvider(Wrist wrist, Supplier<Double> goal) {
    m_wrist = wrist;
  ...
  @Override
  public void execute() {
    m_goal = new State(m_goalSupplier.get(), 0.0);
    ...
    ff=0;
    double pid = m_pid.calculate(m_wrist.getEncoderPosition(), m_goal.position);
    double speed = ff + pid;
